...
function Todo() {
  ...
  function displayTodos(todo) {
    return (
      <div
        ...
        data-todo-isediting={todo.isEditing}
      ...
      >
        <TodoItem>
          <TodoLeft>
            <TodoCheckBox
              className={"todo-box" + todo.id}
              onClick={(e) => handleSetChecked(e, todo.id, todo.isDone)}
              type="checkbox"
            />
            {todo.isEditing ? (
              <TodoEditText
                ref={editTextRef}
                value={editingInputValue || todo.todo}
                onChange={handleSetEditingTodoValue}
              />
            ) : (
  ...
  return (
    <TodoWrapper>
      ...
      <TodoItemSection>{conditionalRender()}</TodoItemSection>
    </TodoWrapper>
  );
}
